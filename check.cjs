...
assert.equal(run('R.filter(r=>matches(r)).length'),24);
...
run("reset();platform='windows'");assert.equal(run('R.filter(r=>matches(r)).length'),12);
...
run('reset()');assert.equal(run('R.filter(r=>matches(r)).length'),24);
